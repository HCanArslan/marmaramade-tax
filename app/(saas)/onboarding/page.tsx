...
function CostsStep({ view }: { view: Awaited<ReturnType<typeof loadOnboarding>> }) {
  ...
  return <form action={costsAction}><p className="eyebrow">Adım 3</p><h2 className="mt-2 text-2xl font-semibold">Ortalama maliyetleri bir kez girin</h2><p className="mt-2 text-sm text-stone-600">Eksik ürünlere uygulanır; mevcut özel maliyetler asla değiştirilmez. Çalışma alanı varsayımı yoksa mevcut özel sürümlerin ortalaması yalnızca öneri olarak gösterilir.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><NumberField name="averageLaborHours" label="Ürün başına ortalama emek süresi" unit="saat" value={d?.averageLaborHours?.toString() ?? view.suggestedCosts.averageLaborHours ?? "1"}/><NumberField name="hourlyLaborValue" label="Saatlik emek değeri" unit="TRY" value={d?.hourlyLabourValue.toString() ?? view.suggestedCosts.hourlyLaborValue ?? "0"}/><NumberField name="packagingCost" label="Ortalama paketleme maliyeti" unit="TRY" value={d?.packagingCost.toString() ?? view.suggestedCosts.packagingCost ?? "0"}/><NumberField name="materialWastagePercentage" label="Malzeme fire oranı" unit="%" value={d?.materialWastagePercentage.toString() ?? view.suggestedCosts.materialWastagePercentage ?? "0"}/><NumberField name="exportHandlingCost" label="Ortalama ihracat / ETGB gideri" unit="TRY" value={d?.exportHandlingCost.toString() ?? "0"}/><NumberField name="monthlyOverhead" label="Aylık genel gider (opsiyonel)" unit="TRY" value={d?.monthlyOverheadKnown ? d.monthlyOverhead.toString() : ""} optional/></div><input type="hidden" name="currency" value="TRY"/><div className="mt-6 rounded-xl bg-stone-50 p-4 text-sm"><p className="font-semibold">Önizleme</p><p className="mt-1 text-stone-600">{view.affectedProductCount} ürün eksik alanlar için bu varsayımları alacak. Malzeme maliyeti bilinmiyorsa eksik kalacak.</p></div><label className="mt-4 flex items-start gap-3 text-sm"><input className="mt-1" type="checkbox" name="confirmApply" value="accepted" required/><span>Yalnızca eksik değerleri doldur.</span></label><button className={`${primary} mt-7`}>Eksiklere uygula <ChevronRight size={16}/></button></form>;
}
