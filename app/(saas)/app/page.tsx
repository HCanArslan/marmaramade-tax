...
export default async function SaasDashboardPage() {
  ...
  const hasCalculatedProducts = view.portfolio.includedCount > 0;
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="İçe aktarılan ürün" value={String(view.products.length)}/><Metric label="Hesaplanan / hariç" value={`${view.portfolio.includedCount} / ${view.portfolio.excludedCount}`}/><Metric label="Brüt gelir" value={money(hasCalculatedProducts ? view.portfolio.totalRevenue : null, currency)}/><Metric label="Ortalama nakit marj" value={view.averageMargin ? `%${view.averageMargin.toDecimalPlaces(1).toFixed(1)}` : "—"}/></div>
    <div className="grid gap-4 lg:grid-cols-3"><Metric label="Nakit kâr" value={money(hasCalculatedProducts ? view.portfolio.cashProfit : null, currency)}/><Metric label="Ekonomik kâr" value={money(hasCalculatedProducts ? view.portfolio.economicProfit : null, currency)}/><Metric label="Zarar eden ürün" value={String(view.lossMaking)}/></div>
