...
export async function testShipEntegraConnectionAction() {
  ...
  try {
    ...
    await prisma.shipEntegraApiCall.create({
      data: {
        operation: "FIND_CARRIERS",
        endpointCategory: "READ_ONLY",
        success: true,
        statusCode: 200,
        durationMs: 0,
      },
    ...
    const status = safe.code === "AUTH_FAILED" ? "AUTH_FAILED" : "DEGRADED";
    await prisma.shipEntegraConnection.upsert({
      where: { environment },
      update: {
        status,
...
const quoteFormSchema = z.object({
  ...
  destinationPostalCode: z.string().trim().optional(),
  lengthCm: z.string().regex(/^\d+(\.\d+)?$/),
  widthCm: z.string().regex(/^\d+(\.\d+)?$/),
  heightCm: z.string().regex(/^\d+(\.\d+)?$/),
  actualWeightKg: z.string().regex(/^\d+(\.\d+)?$/),
});
...
export async function retrieveShipEntegraQuotesAction(formData: FormData) {
  ...
  await retrieveAndPersistShipEntegraQuotes({
    ...input,
    originCountry: "TR",
    expiresInHours: 24,
  });
...
export async function createManualShipEntegraShipmentAction(
  formData: FormData,
) {
  ...
  const value = z
    .object({
      ...
      externalShipmentId: z.string().trim().min(1),
  ...
  const existing = await prisma.shipEntegraShipment.findUnique({
    where: { localOrderId: value.localOrderId },
  });
  if (existing)
    throw new Error("This order already has a ShipEntegra shipment.");
  await prisma.$transaction(async (tx) => {
    await tx.shipEntegraShipment.create({
      data: {
        localOrderId: value.localOrderId,
        externalShipmentId: value.externalShipmentId,
        externalOrderReference: `MANUAL-${value.localOrderId}`,
        shipmentStatus: "MANUAL_ENTRY",
    ...
    await tx.auditLog.create({
      data: {
        entityType: "ShipEntegraShipment",
        entityId: value.localOrderId,
        action: "MANUAL_SHIPMENT_LINKED",
        actor: session.user?.email ?? "ADMIN",
        afterJson: JSON.stringify({
          externalShipmentId: value.externalShipmentId,
          hasTrackingNumber: Boolean(value.trackingNumber),
          actualCost: value.actualCost,
          currency: value.currency,
        }),
      },
