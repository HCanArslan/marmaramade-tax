...
export async function syncEtsyAction(formData: FormData) { await requireAdmin({ redirectTo: "/etsy-import" }); const parsed = z.object({ syncType: z.enum(ETSY_SYNC_TYPES) }).parse({ syncType: formData.get("syncType") }); await syncEtsy(parsed.syncType); revalidatePath("/etsy-import"); }
