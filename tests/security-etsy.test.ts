...
describe("OAuth, tokens and synchronization", () => {
  ...
  it("retains safe Etsy validation details and the failing endpoint", async () => { const error = await EtsyApiError.fromResponse(new Response(JSON.stringify({ error: "min_created is required" }), { status: 400 }), "/v3/application/shops/1/payment-account/ledger-entries"); expect(error.message).toContain("min_created is required"); expect(error.resource).toContain("ledger-entries"); });
