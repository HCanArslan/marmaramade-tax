...
import { EtsyEndpoints } from "@/lib/etsy/endpoints";
import { collectOffsetPages } from "@/lib/etsy/pagination";
import { etsyInventorySchema, etsyLedgerEntrySchema, etsyListingImageSchema, etsyListingSchema, etsyPaymentSchema, etsyReceiptSchema, etsyShopSchema, paginatedSchema } from "@/lib/etsy/schemas";
...
async function importListings(connectionId: string, syncRunId: string, shopId: string, get: <T>(path: string) => Promise<T>) {
  const schema = paginatedSchema(etsyListingSchema); const pages = await collectOffsetPages(async (offset, limit) => schema.parse(await get(EtsyEndpoints.activeListings(shopId, offset, limit))));
  for (const listing of pages.results) {
    ...
    await prisma.etsyListing.upsert({ where: { etsyListingId: String(listing.listing_id) }, update: { syncRunId, sku: listing.skus?.[0], title: listing.title, description: listing.description, state: listing.state, priceAmount: amount(listing.price), priceCurrency: listing.price.currency_code, quantity: listing.quantity, taxonomyId: listing.taxonomy_id ? String(listing.taxonomy_id) : null, url: listing.url, favorerCount: listing.num_favorers, listingType: listing.type, sourceCreatedAt: date(listing.creation_timestamp), sourceUpdatedAt: date(listing.updated_timestamp), sourceEndingAt: date(listing.ending_timestamp), sourceHash, lastImportedAt: new Date(), ...(existing?.sourceHash !== sourceHash ? { lastChangedAt: new Date() } : {}) }, create: { connectionId, syncRunId, etsyListingId: String(listing.listing_id), sku: listing.skus?.[0], title: listing.title, description: listing.description, state: listing.state, priceAmount: amount(listing.price), priceCurrency: listing.price.currency_code, quantity: listing.quantity, taxonomyId: listing.taxonomy_id ? String(listing.taxonomy_id) : null, url: listing.url, favorerCount: listing.num_favorers, listingType: listing.type, sourceCreatedAt: date(listing.creation_timestamp), sourceUpdatedAt: date(listing.updated_timestamp), sourceEndingAt: date(listing.ending_timestamp), sourceHash } });
  ...
  await prisma.etsySyncRun.update({ where: { id: syncRunId }, data: { listingsImported: pages.results.length } });
}
...
async function importLedgerAndPayments(connectionId: string, syncRunId: string, shopId: string, get: <T>(path: string) => Promise<T>) {
  const schema = paginatedSchema(etsyLedgerEntrySchema); const pages = await collectOffsetPages(async (offset, limit) => schema.parse(await get(EtsyEndpoints.ledger(shopId, offset, limit))));
